...
function setRandomRotateImage() {
  ...
  const randomImage = rotateImages[Math.floor(Math.random() * rotateImages.length)];
  const rotateImage = document.getElementById('rotate-image');
  if (rotateImage) {
    rotateImage.src = randomImage;
    rotateImage.alt = '回転促しキャラクター';
  }
}
